  ...
  try {
    ...
    topRow.push(
      <Col key="cori-tile" md={6} style={{ padding: 0 }}>
        <Tile id="tile-1" image={cori.hero ? cori.hero.url : null} width="100%" imageAlign={cori.imageAlign}>
    ...
    topRow.push(
      <Col key="va-tile" md={6} style={{ padding: 0 }}>
        <Tile id="tile-1" image={va.hero ? va.hero.url : null} width="100%" imageAlign={va.imageAlign}>
          <div id="hover" style={{ zIndex: 10, position: 'relative', top: 65, padding: 30 }}>
            <H1>{va.title}</H1>
            <P>{va.subheader}</P>
            <center>
    ...
    areas.map((area, i) => {
      row.push(
        <Col key={`${area.slug}-tile`} md={6} lg={3} style={{ padding: 0 }}>
          <Tile last id={`tile-${i.toString()}`} image={area.hero ? area.hero.url : null} width="100%" imageAlign={area.imageAlign}>
            <HoverContainer>
              <H1>{area.title}</H1>
              <P>{area.subheader}</P>
              <center>
